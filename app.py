...
def get_map():
    ...
    if request.method == 'POST':
        ...
        # Traffic API request
                                                                                                                                    # HERE API Key
        # here_api_key = "zGTdlSedweD6DDZGY1W3Kut8GOO7Su7cg3xBMDF4BYg"
        ...
        # Ensure coordinates are ordered correctly for HERE API (minLon,minLat,maxLon,maxLat)
        min_lat, max_lat = sorted([lat1, lat2])
        min_lng, max_lng = sorted([lng1, lng2])

        # Correct Traffic API URL
        traffic_url = f"https://data.traffic.hereapi.com/v7/flow?locationReferencing=shape&in=bbox:{min_lng},{min_lat},{max_lng},{max_lat}&apiKey={here_api_key}"
        ...
        if 'results' not in traffic_data and 'flow' not in traffic_data:
            print("HERE API Response:", traffic_data)  # Debug print to see actual structure
            return render_template('index.html', error="Unexpected HERE API response. Check your API key or parameters.")
        ...
        client = openrouteservice.Client(key='eyJvcmciOiI1YjNjZTM1OTc4NTExMTAwMDFjZjYyNDgiLCJpZCI6IjNjMTFiOTVhMWFlNzQ2ZTFhMjhlZWJkN2VlNmU5YjRmIiwiaCI6Im11cm11cjY0In0=') # Initialize client
